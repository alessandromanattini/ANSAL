...
#ifndef JucePlugin_PreferredChannelConfigurations
bool SimpleVocoderProcessor::isBusesLayoutSupported(const BusesLayout& layouts) const
{
#if JucePlugin_IsMidiEffect
...
void SimpleVocoderProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    for (int i = 0; i < buffer.getNumSamples(); i++)               // loop over samples
    {
        ...
        for (int lag = 0; lag < 128; lag++)  R[lag] = R[lag] * corr_k + (1- corr_k) * X[pos] * X[rd--]; //leaky autocorrelation
...
void SimpleVocoderProcessor::setCorrK(float newValue) {
    corr_k = newValue;
}

float SimpleVocoderProcessor::getCorrK() {
    return corr_k;
}